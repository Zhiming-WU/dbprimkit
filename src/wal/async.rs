...
//! #[tokio::main]
//! async fn main() {
//!     let inst = TokioFileWalInstance::new("my_wal", "/tmp");
...
use crate::io::{AsyncIoBackend, TokioFileIoBackend};
...
async fn restore_max_lsn<IO: AsyncIoBackend>(
    reader: &mut IO::Reader,
    file_size: u64,
) -> Result<u64> {
    ...
    // if the last block is corrupt, read the last non-corrupt one.
...
async fn new_wal_fields<IO: AsyncIoBackend + Send + Sync>(
    ...
    let (min_lsn, max_lsn) = restore_lsn(&mani_rotator, &data_rotator).await?;
    Ok((
        mani_rotator,
        data_rotator,
        data_rnp,
        ...
        while let Some(reader) = &mut self.reader {
            ...
            match res {
                ...
                Err(err) if err.kind() == ErrorKind::UnexpectedEof => {
                    ...
                    let (rd, _) = open_reader(&self.wal.data_rotator, &rot_name).await?;
                    self.reader = Some(rd);
                    continue;
                }
                Err(err) => {
...
impl<IO: AsyncIoBackend + Send + Sync> WalReader<IO> {
    ...
    pub async fn get_log_stream<'a>(
        ...
        let (mut reader, fsize) = open_reader(&self.data_rotator, &rot_name).await?;
        let mut offset = 0u64;
        let mut bbuf = BytesMut::with_capacity(BLOCK_SIZE);
