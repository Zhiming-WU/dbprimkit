//! A simple WAL (Write Ahead Log) implementation used in async code.
//!
...
//! #[tokio::main]
//! async fn main() {
//!     let inst = TokioFileWalInstance::new("my_wal", "/tmp", None).await.unwrap();
...
//!     let mut stream = reader.get_log_stream(min_lsn).await.unwrap();
...
struct WalDataRotNameProviderImpl<IO: AsyncIoBackend> {
    dir: PathBuf,
    prefix: String,
...
impl<IO: AsyncIoBackend + Send + Sync> WalDataRotNameProviderImpl<IO> {
    ...
    async fn restore_latest_rot_info(&self) -> Result<Option<(u32, u64, String)>> {
...
struct WalDataRotNameProvider<IO: AsyncIoBackend> {
    inner: Arc<WalDataRotNameProviderImpl<IO>>,
}

impl<IO: AsyncIoBackend + Send + Sync> RotNameProvider<IO> for WalDataRotNameProvider<IO> {
    async fn gen_rot_name(&self, rot_no: u32) -> String {
        self.inner.gen_rot_name(rot_no)
    }

    async fn list_rot_names(&self) -> Result<Vec<String>> {
        self.inner.list_rot_names().await
    }

    async fn restore_latest_rot_info(&self) -> Result<Option<(u32, String)>> {
        let res = self.inner.restore_latest_rot_info().await?;
        Ok(res.map(|(rot_no, _next_lsn, name)| (rot_no, name)))
    }
}

async fn read_latest_manifest_entry<IO: AsyncIoBackend>(
...
async fn restore_lsn_from_data<IO: AsyncIoBackend>(
    reader: &mut IO::Reader,
    file_size: u64,
    block_size: u64,
) -> Result<(u64, u64)> {
...
async fn restore_lsn<IO: AsyncIoBackend + Send + Sync>(
    ...
    match mani_rotator.restore_latest_rot(false).await? {
        ...
        Some((mut reader, file_size)) => {
            ...
            let (min_lsn_from_data, max_lsn) = match data_rotator.restore_latest_rot(false).await? {
...
async fn remove_no_use_rots<IO: AsyncIoBackend + Send + Sync>(
    data_rotator: &DefaultRotator<IO, WalDataRotNameProvider<IO>>,
    data_rnp: &WalDataRotNameProviderImpl<IO>,
    min_lsn: u64,
) -> Result<u64> {
    let mut next_no_use_lsn = u64::MAX;
    let names = data_rnp.list_rot_names().await?;
    for (idx, name) in names.iter().enumerate() {
        if idx > 0 {
            let (_, lsn) = data_rnp.parse_rot_name(&name).unwrap();
            if lsn <= min_lsn {
                data_rotator.remove_rot(&names[idx - 1]).await?;
            } else {
                next_no_use_lsn = lsn;
                break;
            }
...
async fn open_reader<IO: AsyncIoBackend + Send + Sync>(
    rotator: &DefaultRotator<IO, WalDataRotNameProvider<IO>>,
    rot_name: &str,
    block_size: u64,
) -> Result<(IO::Reader, u64)> {
    let (reader, fsize) = rotator.open_read_rot(&rot_name, false).await?;
...
async fn new_wal_fields<IO: AsyncIoBackend + Send + Sync>(
    inst: &WalInstance<IO>,
) -> Result<(
    DefaultRotator<IO>,
    ...
    let (min_lsn, max_lsn) =
        restore_lsn(&mani_rotator, &data_rotator, inst.config.block_size as u64).await?;
    Ok((
        mani_rotator,
        data_rotator,
        data_rnp,
...
struct FlushingControlBlock<IO: AsyncIoBackend + Send + Sync> {
    cb: Arc<ControlBlock>,
...
impl<IO: AsyncIoBackend + Send + Sync> FlushingControlBlock<IO> {
    async fn flush_mani_entry(&mut self, min_lsn: u64) -> Result<()> {
        DiskManiEntry::encode(&mut self.mani_buf, min_lsn);
        self.mani_app
            .append(&self.mani_buf.split().freeze(), true)
            .await?;
        self.mani_app.flush().await?;
        // reuse the memory
    ...
    async fn flush_data_blocks(&mut self, rotatable: bool) -> Result<()> {
        self.cb
            .flush_lsn
        ...
        if self.data_app.needs_new_rot().await && min_lsn > self.last_min_lsn {
            self.flush_mani_entry(min_lsn).await?;
        }
        let new_rot = self.data_app.append(&self.data_buf, rotatable).await?;
        if new_rot {
            self.next_no_use_lsn =
                remove_no_use_rots(self.data_app.get_rotator(), &self.data_rnp, min_lsn).await?;
        } else {
...
#[derive(Debug)]
pub struct WalWriter<IO: AsyncIoBackend + Send> {
    inst: WalInstance<IO>,
    cb: Arc<ControlBlock>,
}

impl<IO: AsyncIoBackend + Send + Sync + 'static> WalWriter<IO> {
    async fn new(inst: WalInstance<IO>) -> Result<Self> {
        ...
        tokio::spawn(async move {
            Self::flushing_task(cb, mani_app, data_app, data_rnp).await;
        });
        /*std::thread::spawn(|| {
            ...
            rt.block_on(Self::flushing_task(cb, mani_app, data_app, data_rnp));
        });*/
        Ok(out)
    }

    async fn flushing_task_inner(
        cb: Arc<ControlBlock>,
        mani_app: RotAppender<IO, DefaultRotator<IO>>,
        data_app: RotAppender<IO, DefaultRotator<IO, WalDataRotNameProvider<IO>>>,
        data_rnp: Arc<WalDataRotNameProviderImpl<IO>>,
        ...
        let mut fcb = {
            ...
            let next_no_use_lsn =
                remove_no_use_rots(data_app.get_rotator(), &data_rnp, min_lsn).await?;
            let mut fcb = FlushingControlBlock::<IO> {
                ...
                next_no_use_lsn,
                data_buf: Vec::with_capacity(config.bulk_flush_size as usize),
        ...
        while fcb.cb.running.load(Ordering::Acquire) {
            ...
            }) = fcb.cb.rbuf.pop_for_sc()
            {
                ...
                while !handled {
                    ...
                    if LOG_HEAD_SIZE + left <= bk_left {
                        ...
                        handled = true;
                    } else if LOG_HEAD_SIZE < bk_left
                        && (split || !fcb.data_app.needs_new_rot().await)
                    {
                        // split
                        let fullness = if split {
                            Fullness::Middle
                    ...
                    if fcb.data_buf.len() == fcb.buf_size
                        || (fcb.data_app.needs_new_rot().await
                            && fcb.data_buf.len() & fcb.block_mask == 0)
                    {
                        fcb.fix_block_headers();
                        fcb.flush_data_blocks(!split).await?;
                    }
                }
            }
            ...
            if !fcb.data_buf.is_empty() && fcb.log_check_cnt >= fcb.checks_bef_flush_log {
                ...
                fcb.fix_block_headers();
                fcb.flush_data_blocks(true).await?;
            }
...
pub struct WalReader<IO: AsyncIoBackend + Send + Sync> {
    inst: WalInstance<IO>,
    min_lsn: u64,
    max_lsn: u64,
    data_rotator: DefaultRotator<IO, WalDataRotNameProvider<IO>>,
...
pub struct LogIterator<'a, IO: AsyncIoBackend + Send + Sync> {
    wal: &'a WalReader<IO>,
...
impl<'a, IO: AsyncIoBackend + Send + Sync> LogIterator<'a, IO> {
    async fn get_next(&mut self) -> Option<Result<LogEntry>> {
        loop {
    ...
    async fn read_next(&mut self) -> Result<bool> {
        ...
        while let Some(reader) = &mut self.reader {
            ...
            let res = reader.read_exact(&mut bbuf).await;
            match res {
                ...
                Err(err) if err.kind() == ErrorKind::UnexpectedEof => {
                    ...
                    let (rd, _) =
                        open_reader(&self.wal.data_rotator, &rot_name, block_size as u64).await?;
                    self.reader = Some(rd);
                    continue;
                }
                Err(err) => {
...
impl<IO: AsyncIoBackend + Send + Sync> WalReader<IO> {
    async fn new(inst: WalInstance<IO>) -> Result<Self> {
        let (_, data_rotator, data_rnp, _, min_lsn, max_lsn) = new_wal_fields(&inst).await?;
        Ok(Self {
            inst,
            min_lsn,
            max_lsn,
    ...
    /// Get an [Stream] from which log entriess can be retrieved.
    pub async fn get_log_stream<'a>(
        &'a self,
        start_lsn: u64,
    ) -> Result<impl Stream<Item = Result<LogEntry>> + Unpin> {
        if start_lsn < self.min_lsn || start_lsn > self.max_lsn || self.min_lsn == 0 {
            return Err(InvalidArgument("start_lsn is out of range".into()));
        }

        let mut rot_names = VecDeque::from(self.data_rnp.list_rot_names().await?);
        ...
        let (mut reader, fsize) =
            open_reader(&self.data_rotator, &rot_name, block_size as u64).await?;
        let mut offset = 0u64;
        let mut bbuf = BytesMut::with_capacity(block_size);
        ...
        while offset < fsize && fsize - offset >= block_size as u64 {
            ...
            if header.min_lsn <= start_lsn && start_lsn <= header.max_lsn {
                ...
                if log_iter.cached.is_empty() {
                    if !log_iter.read_next().await? {
                        break;
                    }
                }
                let out = stream! {
                    while let Some(entry) = log_iter.get_next().await {
                        yield entry;
                    }
                };
...
#[derive(Debug)]
pub struct WalInstance<IO: AsyncIoBackend> {
    name: String,
    dir: PathBuf,
    config: TunableConfig,
    _m: PhantomData<IO>,
...
impl<IO: AsyncIoBackend + Send + Sync + 'static> WalInstance<IO> {
    async fn save_config<P: AsRef<Path>>(path: P, config: &TunableConfig) -> Result<()> {
        let opts = OpenOptions::new().with_create(true).with_write(true);
        let mut writer = IO::open_writer(AsRef::<Path>::as_ref(&path), &opts).await?;
        let mut u8_vec = Vec::<u8>::new();
        serde_json::to_writer_pretty(&mut u8_vec, config).map_err(|err| {
            MiscError(format!(
                "Error occurred in saving WAL config, err={:?}",
                err
            ))
        })?;
        writer.write_all(&mut u8_vec).await?;
        writer.sync_all().await?;
        Ok(())
    }

    async fn init_config<P: AsRef<Path>>(
        name: &str,
        dir: P,
        config_arg: Option<TunableConfig>,
    ) -> Result<TunableConfig> {
        let dir_path = dir.as_ref().to_path_buf();
        IO::check_path_is_dir(dir_path.as_path()).await?;
        let conf_path = dir_path.join(format!("{}.cfg", name));
        let opts = OpenOptions::new().with_read(true);
        let config: TunableConfig = match IO::open_reader(conf_path.as_path(), &opts).await {
            Err(IoError(io_e)) if io_e.kind() == ErrorKind::NotFound => {
                let conf = TunableConfig::from_opt(config_arg);
                Self::save_config(conf_path.as_path(), &conf).await?;
                conf
            }
            Ok(mut reader) => {
                let mut u8_vec = Vec::<u8>::new();
                reader.read_to_end(&mut u8_vec).await?;
                let persisted =
                    serde_json::from_reader::<&[u8], TunableConfig>(&u8_vec).map_err(|err| {
                        MiscError(format!(
    ...
    pub async fn new<P: AsRef<Path>>(
        ...
        let config = Self::init_config(name, dir.as_path(), config).await?;
        Ok(Self {
            name: name.into(),
            dir,
            config,
    ...
    /// at the same time.
    pub async fn open_wal_writer(self) -> Result<WalWriter<IO>> {
        WalWriter::<IO>::new(self).await
    }

    /// Open a WAL reader for reading log. Note that WAL reader
    /// and WAL writer of the same WAL instance can not be used
    /// at the same time.
    pub async fn open_wal_reader(self) -> Result<WalReader<IO>> {
        WalReader::<IO>::new(self).await
    }
}

/// The WAL instance type based on [tokio::fs::File] backend.
pub type TokioFileWalInstance = WalInstance<TokioFileIoBackend>;
...
mod tests {
    ...
    async fn do_test_clean(dir: &str, name: &str) {
    ...
    async fn test_wal(
        ...
        let inst = TokioFileWalInstance::new(name, dir.as_path(), config)
            .await
            .unwrap();
        ...
        if do_clean {
            do_test_clean(dir_arg, name).await;
        }
    }
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    #[ignore]
    async fn test_twice_wal_mem_large_adv() {
        test_wal(
            "/dev/shm",
            "test_twice_wal_mem_large_adv_async",
            1000000,
            true,
            false,
            None,
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    #[ignore]
    async fn test_twice_wal_disk_large_adv() {
        test_wal(
            "/tmp",
            "test_twice_wal_disk_large_adv_async",
            1000000,
            true,
            false,
            None,
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn test_wal_config_mem_small_adv() {
        ...
        test_wal(
            "/dev/shm",
            "test_wal_config_mem_small_adv_async",
            10000,
            true,
            true,
            Some(cfg),
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    #[ignore]
    async fn test_wal_config_mem_large_adv() {
        let mut cfg = TunableConfig::default();
        cfg.block_size = 128 * 1024;
        cfg.bulk_flush_size = 512 * 1024;
        cfg.max_data_rot_size = 128 * 1024 * 1024;
        test_wal(
            "/dev/shm",
            "test_wal_config_mem_large_adv_async",
            1000000,
            true,
            true,
            Some(cfg),
